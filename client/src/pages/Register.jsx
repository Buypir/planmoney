...
function Register() {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.error || t('register_error_default'));
        return;
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
      <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 w-96">
        ...
        <label className="block text-sm text-gray-600 dark:text-gray-300 mb-1">{t('register_name_label')}</label>
        <input
          type="text"
          value={name}
        ...
        <label className="block text-sm text-gray-600 dark:text-gray-300 mb-1">{t('register_email_label')}</label>
        <input
          type="email"
          value={email}
        ...
        <label className="block text-sm text-gray-600 dark:text-gray-300 mb-1">{t('register_password_label')}</label>
        <input
          type="password"
          value={password}
